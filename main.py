...
async def cloud(args):
    ...
    for k, v in table.items():
        await restore(restorehost=restorehost,restoreuser=restoreuser,restorepwd=restorepwd, restoreport=restoreport, databases=k, inputpath=outputpath, sqlfile='%s.sql.gz' % v, table=v)   
    end = time.time()
    print(end - start)
